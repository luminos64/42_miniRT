...
void	plane_uv(t_hit hit, float *u, float *v)
{
	...
	// ต้องสร้างแกนสองแกนที่ตั้งฉากกับ normal และตั้งฉากกันเอง
	// เพื่อใช้เป็นแกนอ้างอิงสำหรับคำนวณพิกัด UV บนระนาบ
	arbitrary.x = 0.0f;
	arbitrary.y = 1.0f;
	arbitrary.z = 0.0f;
	...
	// คำนวณ dot product กับแกน u, v แล้วคูณด้วย scale
	// (float ยิ่งมาก ยิ่งลายเล็กลง (ซ้ำมากขึ้น)) เพื่อกำหนดจำนวนครั้งที่ลายซ้ำ
	*u = vec3_dot(local, u_axis) * 0.05f;
	*v = vec3_dot(local, v_axis) * 0.05f;
	// fmod ให้พิกัด UV อยู่ในช่วง [0,1]
	// และทำให้ลาย texture ซ้ำ (tile) ได้อย่างไม่มีที่สิ้นสุดบนระนาบ
	clamp_uv(u, v);
}
